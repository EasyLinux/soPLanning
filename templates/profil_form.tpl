...
<form class="form-horizontal" method="post" action="" target="_blank" name="formUser"  onSubmit="return false;">
	...
	<div class="form-group">
		...
		<div class="col-md-6">
			...
			<label class="radio-inline">
				<input type="radio" id="vueDefautReduit" name="user_prefs_vuedefaut_largereduit" value="vueReduit" {if $user_form.tabPreferences.vueLargeReduit eq "vueReduit"}checked="checked"{/if}>&nbsp;{#menuPlanningReduit#}
			</label>
		</div>
	</div>
	<div class="form-group">
		<div class="col-md-4"></div>
		<div class="col-md-4">
			<br /><input type="button" class="btn btn-primary" value="{#submit#}" onClick="xajax_submitFormProfil('{$user_form.user_id|escape}', $('#email_user').val(), $('#password_tmp').val(), $('#notificationsOui').is(':checked'),$('#vueDefautPersonne').is(':checked'),$('#vueDefautMois').is(':checked'),$('#vueDefautLarge').is(':checked'));"/>
		</div>
	</div>
</form>