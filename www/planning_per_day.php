...
while($ligneTmp = $lines->fetch()) {
	...
	if( $_SESSION['inverserUsersProjets']) {
		$sql = "SELECT planning_periode.*, planning_user.*, planning_projet.createur_id, pc.nom AS nom_createur, pc2.nom AS nom_modifier
				, pl.nom as lieu, pr.nom as ressource
		...
		foreach ($tabTranchesHoraires as $trancheHeureCourante) {
			...
			if (in_array($tmpDate->format('Y-m-d'), $joursFeries)) {
				...
				if($ferieObj->db_load(array('date_ferie', '=', $tmpDate->format('Y-m-d'))) && trim($ferieObj->libelle) != "") {
					...
					$ferie = '<div class="cellHolidays" onmouseover="return coolTip(\'' . addslashes($cooltip) . '\')"  onmouseout="nd()" onClick="event.cancelBubble=true;">' . $smarty->get_config_vars('planning_ferie') . '</div>' . CRLF;
				}
			} else {
				$ferie = false;
			}
			...
			if (isset($joursOccupes[$tmpDate->format('Y-m-d')])) {
				...
				foreach ($joursOccupes[$tmpDate->format('Y-m-d')] as $jour) {
					...
					$html .= '<div id="c_' . $jour['periode_id'] . '_' . $tmpDate->format('Ymd') . '_' . $trancheHeureCourante . '" class="cellProject" onmouseover="cooltip_'.$jour['periode_id'].'()" style="color:' . $couleurTexte . ';' . $opacity . ';background-color:#' . $couleurFond . ';';
					//onMouseUp="Reloader.stopRefresh();modifPeriode(this, ' . $jour['periode_id'] . ');"
...
if($_SESSION['inverserUsersProjets']) {
	...
	$html .= '	<tr>' . CRLF;
	$html .= '		<td class="w25">' . $smarty->get_config_vars('tab_code') . '</td>' . CRLF;
	...
	while($projet = $projets->fetch()) {
		...
		$html .= '<td onmouseover="return coolTip(\'' . addslashes($cooltipProjet) . '\', WIDTH, 400)"  onmouseout="nd()" onClick="javascript:Reloader.stopRefresh();xajax_modifProjet(\'' . $projet->projet_id . '\', \'planning\');undefined;" class="w25 cellProject" style="cursor:pointer;background:#' . $projet->couleur . ';color:' . $couleurTexte .';">' . $projet->projet_id . '</td>' . CRLF;
	...
	while($userTemp = $users->fetch()) {
		...
		$html .= '<td onClick="javascript:Reloader.stopRefresh();xajax_modifUser(\'' . $userTemp->user_id . '\');undefined;" class="w25 cellProject" style="cursor:pointer;background:#' . $userTemp->couleur . ';color:' . $couleurTexte .';">' . $userTemp->user_id . '</td>' . CRLF;
