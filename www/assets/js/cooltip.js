...
//\ coolTip - Core Module v2.0.4- Copyright Robert E Boughner 2004. All rights reserved.
//\   Initial: February 28, 2003 - Last Revised: February 25, 2013
...
//\///////
var FREPLACE=0,FBEFORE=1,FAFTER=2,FALTERNATE=3,FCHAIN=4,cLoaded=0,pmStart=10000000,pmUpper=10001000,pmCount=pmStart+1,pms=new Array(),pmt='',cInfo=new Info(2.04,0),hookPts=new Array(),postParse=new Array(),cmdLine=new Array(),runTime=new Array(),parentLyrs=new Array(),
coreCmds='donothing,sticky,background,noclose,caption,left,right,center,offsetx,offsety,'+
...
'captionclass,closeclass,divclass';registerCommands(coreCmds);var cUdf='undefined';function ud(v){return eval('typeof cd_'+v+'==="undefined"');}
...
'captionclass||closeclass||divclass|';setDefaultVariables(coreValues);if(ud('aboveheight'))var cd_aboveheight=0;if(ud('frame'))var cd_frame=self;if(ud('text'))var cd_text="Default Text";var cTip=null,po,cZindex,cFrame=self;var cOp=(navigator.userAgent.toLowerCase().indexOf('opera')>-1),cMx=cMy=-10000;var docRoot=(cFrame.document.documentElement)?cFrame.document.documentElement:cFrame.document.body;var ctCheckMouseCapture=true;if(window.addEventListener)window.addEventListener("load",ctLoadHandler,false);else if(window.attachEvent)window.attachEvent("onload",ctLoadHandler);if(docRoot.addEventListener)docRoot.addEventListener("mousemove",ctMouseMove,false);else if(docRoot.attachEvent)docRoot.attachEvent("onmousemove",ctMouseMove);var capExtent;function cooltip(){return coolTip(arguments);}
...
return true;}
function cClick(){if(cLoaded){var lyr,ar,po;if(!arguments.length){lyr=(this &&!/window/i.test(this.toString()))?this:cTip;id=lyr.id;}else{ar=arguments[0];if(isType(ar,'string')){id=ar;lyr=fetchObjectLyr(id);}else if(isType(ar,'object')){lyr=ar;id=lyr.id;}}
...
function ctMain(){var layerhtml,cls,ar,lyr;if(!arguments.length){lyr=(this&&!/window/i.test(this.toString()))?this:cTip;id=lyr.id;}else{ar=arguments[0];if(isType(ar,'string')){id=ar;lyr=fetchObjectLyr(id);}else if(isType(ar,'object')){lyr=ar;id=lyr.id;}}
po=lyr.pop;runHook("ctMain",FBEFORE,id);with(po){if(background!=""){layerhtml=runHook('ctContentBackground',FALTERNATE,lyr,text,background);}else{if(cap==""&&!sticky){layerhtml=runHook('ctContentSimple',FALTERNATE,lyr,text);}else{cls=(sticky)?close:'';layerhtml=runHook('ctContentCaption',FALTERNATE,lyr,text,cap,cls);}}
if(sticky){if(timerid>0){clearTimeout(timerid);timerid=0;}
showingsticky=1;removecounter=0;}
if(!runHook("ctCreatePopup",FREPLACE,lyr,layerhtml))return false;if(autostatus>0){status=text;if(autostatus>1)status=cap;}
allowmove=0;if(timeout>0){if(timerid>0)clearTimeout(timerid);timerid=setTimeout("cClick('"+lyr.id+"')",timeout);}
...
function disp(lyr,statustext){var po=lyr.pop;runHook("disp",FBEFORE,lyr,statustext);with(po){if(allowmove==0)  {runHook("placeLayer",FREPLACE,lyr);runHook("showObject",FREPLACE,lyr);allowmove=(!followmouse)?0:1;}}
runHook("disp",FAFTER,lyr,statustext);if(statustext!="")self.status=statustext;}
function ctCreatePopup(lyr,lyrContent){var po=lyr.pop;runHook("ctCreatePopup",FBEFORE,lyr,lyrContent);with(po){if(wrap){var wd,ww;layerWrite(lyr,lyrContent);wd=lyr.offsetWidth;if(wd>(ww=windowWidth())){lyrContent=lyrContent.replace(/\s/g,' ');width=ww;wrap=0;}}
layerWrite(lyr,lyrContent);if(wrap)width=lyr.offsetWidth;}
...
function parseTokens(cObj,pf,ar){var v,i,iN,iM,po,iObj=cObj,nClsOpt=false,cDm,md=-1,par=(pf!='cd_');pf=(par)?'po.':pf;var fnMark=(par&&!ar.length?1:0);if(par)po=cObj.pop;for(i=0;i<ar.length;i++){if(md<0){if(isType(ar[i],'number')&&ar[i]>pmStart&&ar[i]<pmUpper){fnMark=(par?1:0);i--;}else{if(!par)cd_text=unpack(ar[i]);else po.text=unpack(ar[i]);}
...
if(ar[i]==FUNCTION){if(pf=='cd_'){if(!isType(ar[i+1],'number')){v=ar[(++i)];cd_function=isType(v,'function')?v:null;}
}else{fnMark=0;v=null;if(!isType(ar[i+1],'number'))v=ar[(++i)];opt_FUNCTION.call(cObj,v);}
continue;}
...
if(ar[i]==PUID){v=(!isType(ar[i+1],'number')&&ar[i+1]!='')?ar[(++i)]:makeUniqueName('DIV','ctDiv');eval(pf+'puid="'+v+'"');continue;}
...
return iObj;}
function layerWrite(lyr,txt){var po=lyr.pop;if(po&&!po.doXml)txt+="\n";if(po&&po.doXml)lyr=resetNodeContents(txt,lyr);else if(!isUdf(lyr.innerHTML)){lyr.innerHTML='';lyr.innerHTML=txt;}}
...
function wrapStr(lyr,endWrap,fontSizeStr,whichString){var fontStr,po,theClassStr='',theStyleStr='',fontColor,rtnVal,re1=/caption|close|text/i;if(endWrap)rtnVal=(whichString=='close')?'</span>':'</div>';else{po=lyr.pop;with(po){if(re1.test(whichString)){fontStr=eval('po.'+whichString+'font');fontColor=eval('po.'+((whichString=='caption')? 'cap':whichString)+'color');theStyleStr='font-family: '+quoteMultiNameFonts(fontStr)+(whichString=='close'?'':';color: '+fontColor)+';font-size: '+fontSizeStr+';';if(whichString=='close'){if(closeclass){theClassStr='class="'+closeclass+'"';theStyleStr=' float: right;';}else theStyleStr+=' float: right;';}else if(whichString=='caption'){if(bgclass||captionclass){theClassStr='class="'+(bgclass?bgclass:'')+(captionclass?(bgclass?' ':'')+captionclass:'')+'"';theStyleStr="overflow: hidden;";}else{theStyleStr+=' overflow: hidden;background-color: '+bgcolor+';';theStyleStr+=(capalign)?' text-align: '+capalign+';':'';}
}else{if(fgclass||textclass){theClassStr='class="'+(fgclass ?fgclass:'')+(textclass?(fgclass?' ' :'')+textclass:'')+'"';theStyleStr='clear: both;overflow: hidden;';}else
theStyleStr+=(fgcolor)?' background-color: '+fgcolor+';':'';theStyleStr+=(height>0)?' height: '+height+(/[%a-z]+$/i.test(height)?';':'px;'):'';theStyleStr+=(textalign)?' text-align: '+textalign+';':'';theStyleStr+=(/,/.test(cellpad))?setCellPadStr(cellpad):' padding: '+cellpad+'px'+';';theStyleStr='clear: both;overflow: hidden;'+theStyleStr;}}}
theStyleStr=theStyleStr.replace(/;([^\s])/g,';$1');if(theStyleStr&&!/\;$/.test(theStyleStr))theStyleStr=theStyleStr+';';rtnVal=((whichString=='close')?'<span ':'<div ')+(theClassStr?theClassStr:'style="'+theStyleStr+'"')+'>';}
return rtnVal;}
...
function positionDiv(cObj){var po;if(cObj){positionAllPopups();po=cObj.pop;if(po.hoveringSwitch&&cursorOff(cObj)){if(po.delayHide)hideDelay(po.delayHide,cObj);else cClick(cObj.id);po.hoveringSwitch=!po.hoveringSwitch;}}}
...
function createDivContainer(id,frm,zVal){id=(id||makeUniqueName("DIV","ctDiv")),frm=(frm||cFrame),zVal=(zVal||1000);var styStr,divContainer=fetchObjectLyr(id);if(isNull(divContainer)){var body=frm.document.getElementsByTagName("BODY")[0];divContainer=frm.document.createElement("DIV");divContainer.id=id;body.appendChild(divContainer);styStr='position: absolute;visibility: hidden;z-index: '+zVal+';'
divContainer.setAttribute('style',styStr);}
return divContainer;}
function horizontalPlacement(obj,browserWidth,horizontalScrollAmount,widthFix){var placeX,rD=0,ofx,iwidth=browserWidth,winoffset=horizontalScrollAmount;var po=obj.pop;var parsedWidth=(!isNaN(po.width))?parseInt(po.width):obj.offsetWidth;if(!isNull(po.relx)){placeX=po.relx<0?winoffset+po.relx+iwidth-parsedWidth-widthFix:winoffset+po.relx;}else{if(po.hauto==1){ofx=Math.abs(po.offsetx);if((cMx-winoffset)>(iwidth/2)&&cMx-winoffset+(parsedWidth+ofx)>(iwidth-widthFix)){po.hpos=LEFT;rD=1;}else if((cMx-ofx-parsedWidth)<winoffset){po.hpos=RIGHT;rD=1;}
...
return placeX;}
function verticalPlacement(obj,browserHeight,verticalScrollAmount){var placeY,rD=0,ofy,iheight=browserHeight,scrolloffset=verticalScrollAmount;var po=obj.pop;var parsedHeight=(!isNaN(po.aboveheight))?parseInt(po.aboveheight):obj.offsetHeight;if(!isNull(po.rely)){placeY=po.rely<0?scrolloffset+po.rely+iheight-parsedHeight:scrolloffset+po.rely;}else{if(po.vauto==1){ofy=Math.abs(po.offsety);if((cMy-scrolloffset)>(iheight/2)){if(cMy-scrolloffset+parsedHeight+ofy>iheight){po.vpos=ABOVE;rD=1;}
...
return placeY;}
function placeLayer(obj){var placeX,placeY,winoffset,scrolloffset,iwidth,iheight,widthFix=0;if(cFrame.innerWidth)widthFix=20;iwidth=windowWidth();winoffset=(cFrame.pageXOffset)?cFrame.pageXOffset:docRoot.scrollLeft;placeX=runHook('horizontalPlacement',FCHAIN,obj,iwidth,winoffset,widthFix);if(cFrame.innerHeight)iheight=cFrame.innerHeight;else if(docRoot&&isType(docRoot.clientHeight,'number')&&docRoot.clientHeight)iheight=docRoot.clientHeight;scrolloffset=(cFrame.pageYOffset)?cFrame.pageYOffset:docRoot.scrollTop;placeY=runHook('verticalPlacement',FCHAIN,obj,iheight,scrolloffset);repositionTo(obj,placeX,placeY);}
function windowWidth(){var w;if(cFrame.innerWidth)w=cFrame.innerWidth;else if(docRoot&&isType(docRoot.clientWidth,'number')&&docRoot.clientWidth)w=docRoot.clientWidth;return w;}
function setRunTimeVariables(obj){if(!isUdf(runTime)&&runTime.length)for(var k=0;k<runTime.length;k++)runTime[k](obj);}
...
function makeUniqueName(tag,baseName){var objArray,tagCounter=1,i,possName,name,tBody,nameList="";possName=baseName+tagCounter++;tBody=document.getElementsByTagName("BODY")[0];objArray=tBody.getElementsByTagName(tag.toUpperCase());if(objArray.length>0){for(i=0;i<objArray.length;i++){name=objArray[i].getAttribute("name");if(name){nameList+=" "+name+" ";}
name=objArray[i].getAttribute("id");if(name){nameList+=" "+name+" ";}}
while(nameList.indexOf(possName)!=-1){possName=baseName+tagCounter++;}}
return possName;}
...
function registerCommands(cmdStr){if(isType(cmdStr,'string')){var pM=cmdStr.split(',');pms=pms.concat(pM);for(var i=0;i< pM.length;i++)eval(pM[i].toUpperCase()+'='+pmCount++);}}
function registerNoParameterCommands(cmdStr){if(!cmdStr&&!isType(cmdStr,'string'))return;pmt=(!pmt)?cmdStr:pmt+','+cmdStr;}
function registerHook(fnHookTo,fnRef,hookType,optPm){var hookPt;if(fnHookTo=='plgIn'||fnHookTo=='postParse')return;if(isUdf(hookPts[fnHookTo]))hookPts[fnHookTo]=new FunctionReference();hookPt=hookPts[fnHookTo];if(!isNull(hookType)){if(hookType==FREPLACE){hookPt.ovload=fnRef;}
else if(hookType==FBEFORE||hookType==FAFTER){hookPt=(hookType==1?hookPt.before:hookPt.after);if(isType(fnRef,'object'))hookPt=hookPt.concat(fnRef);else hookPt.push(fnRef);if(optPm)hookPt=reOrder(hookPt,fnRef,optPm);}else if(hookType==FALTERNATE){hookPt=hookPt.alt;if(isType(fnRef,'object'))hookPt=hookPt.concat(fnRef);else hookPt.push(fnRef);}else if(hookType==FCHAIN){hookPt=hookPt.chain;if(isType(fnRef,'object'))hookPt=hookPt.concat(fnRef);else hookPt.push(fnRef);}}}
...
function runHook(fnHookTo,hookType){var hookPt=hookPts[fnHookTo],k,reslt=null,arS,ar=arguments;arS=argToString(ar,2);switch(hookType){case FBEFORE:
case FAFTER:
if(!isUdf(hookPt)){hookPt=(hookType==1?hookPt.before:hookPt.after);if(hookPt.length)for(k=0;k<hookPt.length;k++)eval('hookPt[k]('+arS+')');}
break;case FREPLACE:
