...
while($ligneTmp = $lines->fetch()) {
	...
	$html .= '<tr>' . CRLF;
	$html .= '<th id="tdUser_' . ($nbLine-1) . '" ' . ((!is_null($ligneTmp->couleur) && $ligneTmp->couleur != 'FFFFFF') ? ' style="background-color:#'.$ligneTmp->couleur. ';color:' . buttonFontColor('#' . $ligneTmp->couleur) . '"' : '') . '>&nbsp;' . $ligneTmp->nom . '&nbsp;</th>' . CRLF;
	$tmpDate = clone $dateDebut;
...
if($_SESSION['inverserUsersProjets']) {
	...
	//////////////////////////
	$html .= '<table border="0" id="divProjectTable" cellspacing="1" cellpadding="3" class="table-pdf" ' . (isset($_COOKIE['divProjectTable']) && $_COOKIE['divProjectTable'] == 'none' ? 'style="display:none;"' : '') . ' width="' . ($pdf_orientation == 'L' ? 700 : 480) . '" border="1">' . CRLF;
	$html .= '	<tr>' . CRLF;
	...
	while($projet = $projets->fetch()) {
		...
		$html .= '<td>';
		// on charge les p�riodes li�es � ce projet
		$periodes = new GCollection('Periode');
		$sql = "SELECT pp.*
	...
	//////////////////////////
	$html .= '<table border="0" id="divProjectTable" cellspacing="1" cellpadding="3" class="table-pdf" width="' . ($pdf_orientation == 'L' ? 700 : 480) . '" border="1">' . CRLF;
	$html .= '	<tr>' . CRLF;
