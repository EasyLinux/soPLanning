...
function ajoutProjet($origine) {
	...
	$objResponse->addScript('jQuery("#groupe_id").on("select2-selecting", function(e){xajax_autocompleteTitreTache(e.val);});');
	$objResponse->addScript('jQuery("#createur_id").on("select2-selecting", function(e){xajax_autocompleteTitreTache(e.val);});');
...
function ajoutPeriode($dateDebut = '', $ligne_id = '', $periode_id = '', $heureDebut = '') {
	...
	// Initialize select2 box by generic function
	$objResponse->addScript("initselect2('$lang','".$smarty->get_config_vars('choix_option')."')");
	$objResponse->addScript('initmultiselect()');
	// refresh title box when element is selected
...
function modifUser($user_id) {
  require (BASE . '/../includes/minical.inc');
	$objResponse = new xajaxResponse('ISO-8859-1');
	$smarty = new MySmarty();
  ...
  // Ajout Minical
  $smarty->assign('minical',date('Ym'));
  
  $miniCal = new minical($user_id);
  
  $smarty->assign('miniWeeks',$miniCal->getTab(date('Y'),date('m')) );
  $smarty->assign('sMonthYear',$miniCal->getTitle(date('Ym')) );
  
  $smarty->assign('minicalUseable',$miniCal->getUsable() );
  
  $smarty->assign('minicalExceptions', $miniCal->getExceptionsDays(date('Y'), date('m')) );
  // Fin Ajout minical
...
function submitFormUser($user_id, $user_id_origine, $user_groupe_id, $nom, $email, $login, $password, $visible_planningOui, $couleur, $notificationsOui, $envoiMailPwd, $droits, $adresse, $telephone, $mobile, $metier, $commentaire) {
	$objResponse = new xajaxResponse();
	$smarty = new MySmarty();
